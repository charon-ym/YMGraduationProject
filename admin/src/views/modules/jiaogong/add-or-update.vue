...
export default {
	data() {
		...
		return {
			...
			rules: {
				jiaogongzhanghao: [
					{ required: true, message: '教工账号不能为空', trigger: 'blur' },
				],
				mima: [
					{ required: true, message: '密码不能为空', trigger: 'blur' },
				],
				jiaogongxingming: [
					{ required: true, message: '教工姓名不能为空', trigger: 'blur' },
				],
				xingbie: [
				],
				touxiang: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiaogong/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiaogongCrossAddOrUpdateFlag = false;
						 this.parent.search();
